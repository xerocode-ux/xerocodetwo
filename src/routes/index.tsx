...
const WHATSAPP_URL =
  "https://api.whatsapp.com/send/?phone=639562274065&text=Hello%21+Can+I+get+more+info+on+this%3F&type=phone_number&app_absent=0";
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center px-5 pb-14 pt-4 text-center">
        ...
        <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-primary/50 px-5 py-2 text-xs font-bold tracking-[0.15em] text-primary">
          <span className="h-2 w-2 animate-pulse rounded-full bg-primary" />CHANNEL ACTIVE NOW
        </div>
        ...
        <div className="mt-4 grid w-full grid-cols-2 gap-4">
          {FEATURES.map((f) => (
            <div key={f.label} className="rounded-2xl border border-border/70 bg-card px-4 py-5 shadow-sm">
              <div className="text-2xl">{f.icon}</div><div className="mt-2 text-sm font-semibold">{f.label}</div>
            </div>
          ))}
        </div>
        <div className="mt-4 grid w-full grid-cols-3 rounded-2xl border border-border/70 bg-card py-4">
          {STATS.map((s) => <div key={s.label}><div className="text-lg font-extrabold text-gold">{s.value}</div><div className="text-[11px] uppercase tracking-wide text-muted-foreground">{s.label}</div></div>)}
        </div>
        <div className="mt-4 w-full rounded-2xl border border-border/70 bg-card px-5 py-4">
          <p className="text-sm text-muted-foreground">WhatsApp is ready — or tap above</p>
          <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-secondary"><div className="h-full rounded-full bg-primary" style={{ width: "100%" }} /></div>
        </div>
